...
# 关键修复：
# - 启动后前台阻塞，直到 Ctrl+C/HUP/菜单3 停止（不再“回到菜单”或直接回到 shell）
# - 退出统一清理全部进程树（含 watchdog 及其重启的子进程）
# - 修正 2>/dev/null 重定向错别字；?nocache/&nocache 智能拼接
# - 数字着色、URL 全显；作者/探针标红；上传地址默认全量随机

set -Euo pipefail

# =================== cgroup 周期（更平滑） ===================
CGROUP_PERIOD_US=${CGROUP_PERIOD_US:-20000}  # 1–100000 建议
# ============================================================
...
# 启动后是否“前台阻塞直至停止”
FOLLOW_AFTER_START=${FOLLOW_AFTER_START:-1}
...
# 线程PID映射 & 心跳 & 重启计数（注意：watchdog 在子进程中维护）
...
append_nocache() {
  local url="$1" tag="$2"
  local suffix="nocache=$(date +%s%N)-${tag}-$RANDOM"
  if [[ "$url" == *\?* ]]; then
    echo "${url}&${suffix}"
  else
    echo "${url}?${suffix}"
...
print_dl_status() {
  ...
  local _; _=$(atomic_add "$DL_TOTAL_FILE" "$bytes")
...
download_worker() {
  ...
  while true; do
    ...
    local final; final="$(append_nocache "$url" "dl-$id")"
...
upload_worker() {
  ...
  while true; do
    ...
    local final; final="$(append_nocache "$url" "ul-$id")"
...
start_consumption() {
  ...
  echo "[*] 定时汇总：$( ((SUMMARY_INTERVAL>0)) && echo "关闭后可再设每 ${SUMMARY_INTERVAL}s" || echo "关闭" )"
...
# 前台阻塞直到停止（不再回菜单/不回到 shell）
foreground_hold() {
  if [[ -n "${WATCHDOG_PID:-}" ]] && kill -0 "$WATCHDOG_PID" 2>/dev/null; then
    while kill -0 "$WATCHDOG_PID" 2>/dev/null; do sleep 1; done
  else
    # 无 watchdog 时，盯任意子进程
    while :; do
      local alive=0
...
stop_consumption() {
  ...
  kill_scheduler
  limit_apply_percent 100  # 恢复 CPU
  ...
  if ((${#PIDS[@]}==0)); then
    ...
    sleep 0.4
    for pid in "${PIDS[@]}"; do
      safe_pkill_children KILL "$pid" 2>/dev/null || true
...
interactive_start() {
  ...
  if [[ "$MODE" != "u" ]]; then
    list_download_urls; read -rp "下载地址编号（逗号分隔，留空=全量随机）: " pick_dl || true
  ...
  # 关键：开始后前台阻塞，直到停止（不再回菜单/不回 shell）
...
exit_cleanup() {
  ...
  SUPPRESS_CPUWARN_ON_EXIT=1
  stop_consumption
  # 固定四行样式（含最终汇总已由 stop_consumption 输出）
  # 已符合你的要求：
  # [!] 捕获到信号，正在清理…
  # [*] YYYY-mm-dd HH:MM:SS 正在停止全部线程…
  # [*] 最终汇总：下载总计 X.XX MB；上传总计 X.XX MB
  # [+] 已全部停止。
  exit 0
}
...
menu() {
  while true; do
    ...
    case "${c:-}" in
      1) interactive_start        ;; # 开始后会前台阻塞并 exit
      2) configure_cpu_limit_mode ;;
      3) stop_consumption         ;;
      4) echo; list_download_urls; echo; list_upload_urls ;;
      5) echo "当前定时汇总：$([[ $SUMMARY_INTERVAL -gt 0 ]] && echo \"每 ${SUMMARY_INTERVAL}s\" || echo '关闭')"; read -rp "输入间隔秒（0=关闭，留空不变）: " v || true; [[ -z "${v// /}" ]] || { if [[ "$v" =~ ^[0-9]+$ ]]; then SUMMARY_INTERVAL="$v"; stop_summary; (( SUMMARY_INTERVAL > 0 )) && start_summary; echo "${C_GREEN}[+] 定时汇总已设置为：$([[ $SUMMARY_INTERVAL -gt 0 ]] && echo \"每 ${SUMMARY_INTERVAL}s\" || echo '关闭')${C_RESET}"; else echo "${C_YELLOW}输入无效。${C_RESET}"; fi; } ;;
      6) configure_limits         ;;
