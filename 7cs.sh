#!/usr/bin/env bash
# 流量消耗/测速脚本（分块/仅脚本限速/CPU行为模拟/熔断与看门狗/定时汇总/强力清理/纯文本主菜单）
# - 主菜单纯文本（避免乱码）
# - 行输出固定列宽 & 彩色数字 & URL 全量显示
# - 停滞/超时杀手、端点熔断、线程看门狗、IPv4/IPv6 自动探测
# - Ctrl+C / SIGHUP 等都会完整清理：杀线程 → 杀进程组 → 打印最终汇总 → 退出

set -Euo pipefail
set +m  # 关闭作业控制，确保子进程位于同一前台进程组，信号可级联
...
NONE_WARNED=0             # "none" 警告是否已提示过
SUPPRESS_WARNINGS=0       # 抑制 CPU 警告（清理时使用）

# 看门狗
WATCHDOG_PID=

# 进程组
SCRIPT_PGID="$(ps -o pgid= -p $$ 2>/dev/null | tr -d ' ' || echo "$$")"
...
# UI 辅助
#############################
term_cols() { command -v tput >/dev/null 2>&1 && tput cols || echo 120; }
...
atomic_add() {
  local file="$1" add="$2"
  if command -v flock >/div/null 2>&1; then :; fi  # 占位，下一行包含真正逻辑（避免某些 shell 提示）
...
print_dl_status() {
  ...
  local _=; _=$(atomic_add "$DL_TOTAL_FILE" "$bytes")  # 更新全局累计
  : > "$COUNTER_DIR/hb.dl.$tid"
  local rate; rate=$(fmt_rate "$bytes" "$sec")
  printf "%s[DL#%s]%s | 本次 %s%6.2f MB%s | 累计 %s%7.2f MB%s | 速率 %s%6s MB/s%s | %s%s%s\n" \
    "${C_CYAN}" "$tid" "${C_RESET}" \
...
print_ul_status() {
  ...
  local rate; rate=$(fmt_rate "$bytes" "$sec")
  printf "%s[UL#%s]%s | 本次 %s%6.2f MB%s | 累计 %s%7.2f MB%s | 速率 %s%6s MB/s%s | %s%s%s\n" \
...
limit_apply_percent() {
  ...
  case "$LIMIT_METHOD" in
    ...
    *)
      if (( SUPPRESS_WARNINGS==0 && NONE_WARNED==0 )); then
...
start_consumption() {
  ...
  echo "[*] 定时汇总：$( ((SUMMARY_INTERVAL>0)) && echo "关闭" || echo "关闭" )"  # 保持提示一致

  if (( LIMIT_MODE>0 )); then detect_limit_method; [[ "$LIMIT_METHOD" == "cgroup" ]] && cgroup_enter_self; fi

  if [[ "$MODE" != "u" ]] && (( dl_n > 0 )); then
    for ((i=1; i<=dl_n; i++)); do : > "$COUNTER_DIR/hb.dl.$i"; spawn_dl "$i"; sleep 0.02; done
  fi
  if [[ "$MODE" != "d" ]] && (( ul_n > 0 )); then
    for ((i=1; i<=ul_n; i++)); do : > "$COUNTER_DIR/hb.ul.$i"; spawn_ul "$i"; sleep 0.02; done
  fi
...
stop_consumption() {
  local quiet="${1:-0}"   # 1=静默（供 Ctrl+C 清理使用）
  local skip_counter_cleanup="${2:-0}"  # 1=保留计数文件（不在此函数删）
  # 停调度与限 CPU（清理时抑制告警）
  stop_normal_usage
  SUPPRESS_WARNINGS=1
  ...
  if ! is_running; then
    (( quiet==0 )) && echo "${C_YELLOW}[*] 当前没有运行中的线程。${C_RESET}"
  else
    (( quiet==0 )) && echo "${C_BOLD}[*] $(human_now) 正在停止全部线程…${C_RESET}"
    # 先优雅终止每个工作线程
    for pid in "${PIDS[@]}"; do
    ...
    # 兜底：按进程组杀（包括任何残留 curl）
    kill -- -"$SCRIPT_PGID" 2>/dev/null || true
  ...
  # 记录最终字节（供外部优雅打印）
  local dl_g=0 ul_g=0
  [[ -f "$DL_TOTAL_FILE" ]] && dl_g=$(cat "$DL_TOTAL_FILE" 2>/dev/null || echo 0)
  [[ -f "$UL_TOTAL_FILE" ]] && ul_g=$(cat "$UL_TOTAL_FILE" 2>/dev/null || echo 0)
  LAST_DL_BYTES="$dl_g"
  LAST_UL_BYTES="$ul_g"
  ...
  (( skip_counter_cleanup==0 )) && cleanup_counters
  (( quiet==0 )) && echo "${C_GREEN}[+] 已全部停止。${C_RESET}"
}
...
interactive_start() {
  ...
  if [[ -z "${hours// /}" ]]; then
    ...
    END_TS=$(( $(date +%s) + secs )); echo "[*] 预计停止于：$(date_human_ts "$END_TS")"
  else
...
cleanup_and_exit() {
  ...
  # 静默停止（保留计数文件），以便我们在此处统一打印“最终汇总”
  stop_consumption 1 1
  local dl_mb ul_mb
  dl_mb=$(bytes_to_mb "$LAST_DL_BYTES"); ul_mb=$(bytes_to_mb "$LAST_UL_BYTES")
  echo "[*] 最终汇总：下载总计 ${dl_mb} MB；上传总计 ${ul_mb} MB"
  cleanup_counters
  echo "[+] 已全部停止。"
  # 兜底再杀进程组一次
  kill -- -"$SCRIPT_PGID" 2>/dev/null || true
