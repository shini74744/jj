...
# - 菜单去美化（主菜单使用纯文本，避免乱码）
# - 行输出：固定列宽（本次/累计/速率/URL 中间省略）
# - 停滞/超时杀手、端点熔断、线程看门狗、IPv4/IPv6 自动探测
# - 任意退出路径（0 退出、Ctrl+C、TERM、异常）都彻底清理所有子进程（curl 线程）、cpulimit watcher、调度器、看门狗与临时目录
...
# 新增：给“单个新 PID”挂 cpulimit（用于看门狗重启/新 spawn 的线程）
...
spawn_dl() {
  ...
  DL_PIDS["$id"]=$pid; PIDS+=("$pid")
  # 若处于 cpulimit 恒定 50% 模式，给“新 PID”立即挂限额
  if [[ "$LIMIT_METHOD" == "cpulimit" && "$LIMIT_MODE" -eq 1 ]]; then
    local per; per=$(calc_cpulimit_per_proc 50)
    cpulimit_apply_for_pid "$pid" "$per"
  fi
}
spawn_ul() {
  local id="$1"
  upload_worker "$id" & local pid=$!
  UL_PIDS["$id"]=$pid; PIDS+=("$pid")
  if [[ "$LIMIT_METHOD" == "cpulimit" && "$LIMIT_MODE" -eq 1 ]]; then
    local per; per=$(calc_cpulimit_per_proc 50)
    cpulimit_apply_for_pid "$pid" "$per"
  fi
...
on_int_term() {
  echo; echo "${C_YELLOW}[!] 捕获到信号，正在清理…${C_RESET}"
  cleanup_all
  exit 0
}
on_exit() {
