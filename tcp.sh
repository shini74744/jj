...
# 版本: 1.6.3 (微调优化版)
# 变更说明 (v1.6.3):
# - 修复幂等性检测标记, 避免重复应用同一配置
# - 在配置文件头部增加明确标记 bbr_tuned_by_script=1
# - 写入配置前确保 /etc/sysctl.d 目录存在
# ==============================================================================
...
show_tips() {
    ...
    local bak_file_hint
    bak_file_hint=$(ls -t "$CONF_FILE.bak_"* 2>/dev/null | head -n 1 || true)
...
check_if_already_applied() {
    if grep -q "bbr_tuned_by_script=1" "$CONF_FILE" 2>/dev/null; then
        local current_cc
        current_cc=$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || echo "")
...
revert_optimizations() {
    ...
    local latest_backup
    latest_backup=$(ls -t "$CONF_FILE.bak_"* 2>/dev-null | head -n 1 2>/dev/null || true)
