...
# - 菜单去美化（主菜单使用纯文本，避免乱码）
# - 行输出：固定列宽（本次/累计/速率/URL 完整显示）
# - 停滞/超时杀手、端点熔断、线程看门狗、IPv4/IPv6 自动探测
# - Ctrl+C / SIGHUP（SSH断开） 完全退出并清理所有线程、cpulimit watcher、调度器与临时目录
...
# 上传地址池（优先快速：Cloudflare；附加两个备用可用）
UPLOAD_URLS=(
  "https://speed.cloudflare.com/__up"
  "https://fancy-feather-6ba1.7474483.workers.dev/"
  "https://upload.cpp.sh/"   # 轻量上传回显
)
...
print_dl_status() {
  ...
  printf "${C_CYAN}[DL#%s]${C_RESET} | 本次 ${C_YELLOW}%6.2f MB${C_RESET} | 累计 ${C_GREEN}%7.2f MB${C_RESET} | 速率 ${C_YELLOW}%6s MB/s${C_RESET} | %s\n" \
    "$tid" "$(fmt_mb "$bytes")" "$(fmt_mb "$tsum")" "$rate" "$url"
}
print_ul_status() {
  local tid="$1" url="$2" bytes="$3" tsum="$4" sec="${5:-0}"
  ...
  local rate; rate=$(fmt_rate "$bytes" "$sec")
  printf "${C_MAGENTA}[UL#%s]${C_RESET} | 本次 ${C_YELLOW}%6.2f MB${C_RESET} | 累计 ${C_GREEN}%7.2f MB${C_RESET} | 速率 ${C_YELLOW}%6s MB/s${C_RESET} | %s\n" \
    "$tid" "$(fmt_mb "$bytes")" "$(fmt_mb "$tsum")" "$rate" "$url"
}
...
safe_pkill_children() {
  ...
  if command -v pkill >/dev/null 2>&1; then
    ...
    # 注意：重定向要在命令末尾，避免被当成参数
    local kids; kids=$(ps -o pid= --ppid "$ppid" 2>/dev/null || true)
    [[ -n "${kids:-}" ]] && { echo "$kids" | xargs -r kill -"${sig}" 2>/dev/null || true; }
  fi
}
...
download_worker() {
  ...
  while true; do
    ...
    local final="${url}?nocache=$(date +%s%N)-dl-${id}-${RANDOM}"
...
upload_worker() {
  ...
  while true; do
    ...
    local final="${url}?nocache=$(date +%s%N)-ul-${id}-${RANDOM}"
...
start_consumption() {
  ...
  echo "${C_BOLD}[*] 作者: ${C_RED}${AUTHOR}${C_RESET} | 探针: ${C_RED}${PROBE_HOST}${C_RESET}"
  echo "[*] 定时汇总：$( ((SUMMARY_INTERVAL>0)) && echo "关闭后可设定" || echo "关闭" )"
...
interactive_start() {
  ...
  # 前台阻塞：开始后不再重绘菜单，直到 Ctrl+C 或 HUP
  while is_running; do sleep 3600; done
}
...
# INT/TERM/HUP 都触发优雅清理（HUP=SSH 断开时）
trap on_interrupt INT TERM HUP
# 退出时尝试清理我们创建的 cgroup 目录（不影响其它功能）
...
menu() {
  while true; do
    ...
    case "${c:-}" in
      1) interactive_start        ;;  # 开始后前台阻塞，直至 Ctrl+C/HUP
