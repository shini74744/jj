...
# 连通性与下载/上传行为
IP_VERSION=${IP_VERSION:-4}     # 4=仅IPv4, 6=仅IPv6, 0=自动（本版新增自动探测）
FORCE_HTTP1=${FORCE_HTTP1:-1}   # 1=强制 HTTP/1.1
ALWAYS_CHUNK=${ALWAYS_CHUNK:-1} # 1=总是按固定分块；0=先整段→失败再兜底分块（本版为整段加保险）
...
# 轻量 IPv6/IPv4 自动探测（当 IP_VERSION=0）
...
NONE_WARNED=0             # LIMIT_METHOD=none 时仅提示一次
...
date_human_ts() {
  ...
  if date -d @0 +%F >/dev/null 2>&1; then
    date -d @"$1" "+%F %T"
  # BSD/macOS date
  elif date -r 0 +%F >/dev/null 2>&1; then
    date -r "$1" "+%F %T"
  else
    # 兜底：直接回显 epoch
    echo "$1"
...
curl_measure_full() {
  ...
  local extra=(); (( limit_bps > 0 )) && extra+=(--limit-rate "$limit_bps")
  local max_bytes=$((128*1024*1024)) # 128MB 安全阈值
  curl -sS -L \
    "${CURL_IP_OPT[@]}" "${HTTP_VER_OPT[@]}" \
...
cpulimit_apply_for_pids() {
  ...
  local inc_child_opt=""
  if cpulimit -h 2>&1 | grep -qE -- '-z|children'; then
    inc_child_opt="-z"   # 常见变体：-z 表示包含子进程
  fi
  ...
  case "$LIMIT_METHOD" in
    ...
    *)
      if (( NONE_WARNED == 0 )); then
        echo "${C_YELLOW}[!] 本机无 cgroup v2 写权限也未安装 cpulimit，无法精确限速（仅提示一次）。${C_RESET}"
        NONE_WARNED=1
      fi
      ;;
  esac
...
interactive_start() {
  ...
  if [[ -z "${hours// /}" ]]; then
    ...
    END_TS=$(( $(date +%s) + secs )); echo "[*] 预计运行 ${hours} 小时，至 $(date_human_ts "$END_TS") 停止。"
  else
