...
# 变更：
# - 分块分离(CHUNK_MB_DL/UL)、默认仅CF上行(留空时)、智能分配线程（偶数均分/奇数自动偏向更受限的一方或默认给上传）
# - 新增：菜单 2「限制模式（CPU）」：
#   1) 恒定 50%
#   2) 模拟正常使用（随机波动/偶发短突发/夜间低负载）
#   3) 清除
#   说明：2 模式替换原“5min@90% / 10min@50%”，更贴近日常使用并降低风控命中。
...
CPULIMIT_ATTACH_MODE=${CPULIMIT_ATTACH_MODE:-by_exec}  # by_exec | by_pids
...
format_ts() {
  local ts="$1"
  date -d @"$ts" "+%F %T" 2>/dev/null || \
  date -r  "$ts" "+%F %T" 2>/dev/null || \
  echo "$ts"
}

cores_count() {
  if command -v nproc >/dev/null 2>&1; then nproc; return; fi
  if getconf _NPROCESSORS_ONLN >/dev/null 2>&1; then getconf _NPROCESSORS_ONLN; return; fi
  if sysctl -n hw.ncpu >/dev/null 2>&1; then sysctl -n hw.ncpu; return; fi
  echo 1
}

auto_threads() {
  local cores; cores=$(cores_count)
  local t=$(( cores * 2 ))
  (( t < 4 )) && t=4
  (( t > 32 )) && t=32
  echo "$t"
...
show_status() {
  ...
  printf " 汇总: %s 结束: %s\n" \
    ...
    "$( ((END_TS>0)) && format_ts "$END_TS" || echo "手动停止" )"
  printf " 限制模式: %s（方法: %s）\n" \
    "$( case "$LIMIT_MODE" in 0) echo "关闭" ;; 1) echo "恒定 50%" ;; 2) echo "模拟正常使用" ;; esac )" \
    "$( case "$LIMIT_METHOD" in "") echo "-" ;; *) echo "$LIMIT_METHOD" ;; esac )"
  echo "${C_BOLD}└────────────────────────────────────────────────────────┘${C_RESET}"
}

parse_choice_to_array() {
  local input="$1" src="$2" dst="$3"
  local -n _SRC="$src"; local -n _DST="$dst";
  _DST=()
...
upload_worker() {
  ...
  while true; do
    ...
    if [[ -n "$res" ]]; then bytes="${res%% *}"; code="${res##* }"; fi
...
# 进入 cgroup（确保 +cpu）
cgroup_enter_self() {
  if [[ "$LIMIT_METHOD" != "cgroup" ]]; then return 0; fi
...
cpulimit_apply_for_pids() {
  ...
  if [[ "$CPULIMIT_ATTACH_MODE" == "by_exec" ]]; then
    # 按进程名限制所有 curl（最贴合本脚本负载）。如需仅限制本脚本子进程，请改为 by_pids。
    cpulimit -e curl -l "$pct" -b >/dev/null 2>&1 &
    CPULIMIT_WATCHERS+=("$!")
  else
    ((${#PIDS[@]})) || return 0
    for pid in "${PIDS[@]}"; do
...
normal_usage_scheduler() {
  ...
  while true; do
    ...
    if (( cycle % $(rand_between 6 10) == 0 )); then
      ...
      limit_apply_percent "$target"; sleep "$dur"; continue
    fi
...
limit_set_mode() {
  ...
  case "$mode" in
    0)
      LIMIT_MODE=0; stop_normal_usage; kill_cpulimit_watchers; limit_apply_percent 100
      echo "${C_GREEN}[+] 已清除 CPU 限制。${C_RESET}"
      ;;
    1)
      LIMIT_MODE=1; stop_normal_usage; [[ "$LIMIT_METHOD" == "cgroup" ]] && cgroup_enter_self; limit_apply_percent 50
      echo "${C_GREEN}[+] 已设置恒定 50% CPU 限制。${C_RESET}"
      ;;
    2)
      LIMIT_MODE=2; [[ "$LIMIT_METHOD" == "cgroup" ]] && cgroup_enter_self; start_normal_usage
      ;;
  esac
}
...
ensure_limit_on_current_run() {
  ...
  if [[ "$LIMIT_METHOD" == "cgroup" ]]; then
    ...
    if (( LIMIT_MODE==1 )); then local per; per=$(calc_cpulimit_per_proc 50); cpulimit_apply_for_pids "$per"; fi
    if (( LIMIT_MODE==2 )); then start_normal_usage; fi
  fi
}
...
wait_first_output() {
  ...
  for ((i=0; i<START_WAIT_SPINS; i++)); do [[ -f "$COUNTER_DIR/first.tick" ]] && return 0; sleep 0.1; done
}
...
smart_split_threads() {
  ...
  if (( dl_pos==1 || ul_pos==1 )); then
    ...
    else extra_side=$(awk -v dl="$DL_LIMIT_MB" -v ul="$UL_LIMIT_MB" 'BEGIN{if(dl<ul) print "dl"; else print "ul"}')
    fi
  fi
  if [[ "$extra_side" == "dl" ]]; then DL_THREADS=$(( total/2 + 1 )); UL_THREADS=$(( total - DL_THREADS ))
  else UL_THREADS=$(( total/2 + 1 )); DL_THREADS=$(( total - UL_THREADS ))
  fi
}
...
start_consumption() {
  ...
  (( END_TS > 0 )) && echo "[*] 预计结束于：$(format_ts "$END_TS")"
...
interactive_start() {
  ...
  local total_threads
  if [[ -z "${t// /}" ]]; then
    total_threads=$(auto_threads); echo "[*] 自动选择线程数：$total_threads"
  elif [[ "$t" =~ ^[0-9]+$ ]] && (( t > 0 )); then
    total_threads="$t"
  else
    echo "${C_YELLOW}[!] 非法输入，使用自动选择。${C_RESET}"; total_threads=$(auto_threads)
  fi
  ...
  if [[ -z "${hours// /}" ]]; then
    ...
    END_TS=$(( $(date +%s) + secs ))
    echo "[*] 预计运行 ${hours} 小时，至 $(format_ts "$END_TS") 停止。"
  else
...
configure_limits() {
  ...
  case "${sub:-}" in
    1)
      ...
      [[ -z "${v// /}" ]] && { echo "未更改。"; return; }
      if [[ "$v" =~ ^[0-9]+([.][0-9]+)?$ ]]; then UL_LIMIT_MB="$v"; echo "${C_GREEN}[+] 已设置上传总速为 ${UL_LIMIT_MB} MB/s。${C_RESET}"; else echo "${C_YELLOW}[-] 输入无效。${C_RESET}"; fi
      ;;
    2)
      read -rp "输入下载总速（MB/s，0 取消限速）: " v || true
      [[ -z "${v// /}" ]] && { echo "未更改。"; return; }
      if [[ "$v" =~ ^[0-9]+([.][0-9]+)?$ ]]; then DL_LIMIT_MB="$v"; echo "${C_GREEN}[+] 已设置下载总速为 ${DL_LIMIT_MB} MB/s。${C_RESET}"; else echo "${C_YELLOW}[-] 输入无效。${C_RESET}"; fi
      ;;
    3)
      read -rp "输入上下行总速（MB/s，0 取消限速）: " v || true
      [[ -z "${v// /}" ]] && { echo "未更改。"; return; }
      if [[ "$v" =~ ^[0-9]+([.][0-9]+)?$ ]]; then DL_LIMIT_MB="$v"; UL_LIMIT_MB="$v"; echo "${C_GREEN}[+] 已将上下行总速都设为 ${v} MB/s。${C_RESET}"; else echo "${C_YELLOW}[-] 输入无效。${C_RESET}"; fi
      ;;
    4)
...
configure_cpu_limit_mode() {
  echo "${C_BOLD}限制模式（CPU）：${C_RESET}当前 = $( case "$LIMIT_MODE" in 0) echo "关闭" ;; 1) echo "恒定 50%" ;; 2) echo "模拟正常使用" ;; esac )"
  echo " 1) 限制到 50% 以下（恒定）"
  ...
  echo "${C_DIM}说明：优先使用 cgroup v2（需可写 /sys/fs/cgroup），否则回退到 cpulimit；模拟模式将周期性随机调整 CPU 目标，包含短突发与休息窗口。${C_RESET}"
}
